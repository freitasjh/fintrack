        ...
        <div class="flex flex-column align-items-center justify-content-center">
            ...
            <div
                ...
                <div class="w-full surface-card py-8 px-5 sm:px-8" style="border-radius: 53px">
                    ...
                    <div>
                        <label for="email1" class="block text-900 text-xl font-medium mb-2">{{ $t('loginUsername')
                            }}</label>
                        <InputText id="email1" type="text" :placeholder="$t('emailAddress')"
                            class="w-full md:w-30rem mb-5" style="padding: 1rem" v-model="login.username" />

                        <label for="password1" class="block text-900 font-medium text-xl mb-2">{{ $t('password')
                            }}</label>
                        <Password id="password1" v-model="login.password" :placeholder="$t('password')"
                        ...
                        <div class="flex align-items-center justify-content-between mb-5 gap-5">
                            <div class="flex align-items-center">
                                <a @click="router.push('/newAccount')"
                                    class="font-medium no-underline ml-2 text-right cursor-pointer"
                                    style="color: var(--primary-color)">{{ $t('newAccount') }}</a>
                            </div>
                            <a class="font-medium no-underline ml-2 text-right cursor-pointer"
                                style="color: var(--primary-color)">{{ $t('forgotPassword') }}</a>
                        </div>
                        <Button :label="$t('signIn')" class="w-full p-3 text-xl" @click="authenticate"></Button>
                    </div>
                </div>
            </div>
        </div>
